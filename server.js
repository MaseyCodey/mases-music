...
app.get('/api/tracks',async(req,res)=>{const items=await catalog();await syncTracks(items);const liked=req.user&&!req.user.isAdmin?new Set((await query('SELECT track_id FROM likes WHERE user_id=$1',[req.user.id])).map(x=>String(x.track_id))):new Set();res.json({tracks:items.map(x=>({...x,audioUrl:`https://raw.githubusercontent.com/${GH_REPO}/${encodeURIComponent(GH_BRANCH)}/${x.path.split('/').map(encodeURIComponent).join('/')}`,liked:liked.has(String(x.id))}))})});
